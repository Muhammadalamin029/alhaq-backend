...
class OrderService:
    ...
    def get_orders_by_seller(self, db: Session, seller_id: UUID, limit: int = 10, page: int = 1, status: Optional[str] = None) -> Tuple[List[Order], int]:
        ...
        for order in orders:
            ...
            order.total_amount = sum(item.quantity * item.price for item in seller_items)
    ...
    def update_seller_items_status(
        ...
        try:
            with self.transaction_context(db):
                ...
                if not self.validate_status_transition(current_item_status, new_status):
                    valid_transitions = self.get_valid_status_transitions(current_item_status)
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
    ...
    def update_order_status(
        ...
        try:
            with self.transaction_context(db):
                ...
                # Authorization check
                if user_role == "seller":
                    # For sellers, use the seller-specific item status update method
