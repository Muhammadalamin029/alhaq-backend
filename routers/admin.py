...
class CreateAdminRequest(BaseModel):
    ...
    description: str = Field(
        default="System Administrator", description="Admin description"
    )
...
async def get_admin_dashboard_stats(
    ...
    try:
        ...
        completed_payments_query = db.query(Payment).filter(
            Payment.status == "completed"
        )
        if range_start:
            completed_payments_query = completed_payments_query.filter(
                Payment.created_at >= range_start
            )

        total_revenue = (
            completed_payments_query.with_entities(func.sum(Payment.amount)).scalar()
            or 0
        )
        ...
        new_orders_today = (
            db.query(Order).filter(func.date(Order.created_at) == today).count()
        )

        revenue_today = (
            db.query(func.sum(Payment.amount))
            .filter(
                func.date(Payment.created_at) == today, Payment.status == "completed"
            )
            .scalar()
            or 0
        )
        ...
        # Status counts
        locked_users = (
            db.query(User).filter(User.locked_until > datetime.utcnow()).count()
        )

        out_of_stock_products = (
            db.query(Product).filter(Product.stock_quantity == 0).count()
        )

        pending_orders = (
            db.query(Order).filter(Order.status == ORDER_STATUS_PENDING).count()
        )
        processing_orders = (
            db.query(Order).filter(Order.status == ORDER_STATUS_PROCESSING).count()
        )
        paid_orders = db.query(Order).filter(Order.status == ORDER_STATUS_PAID).count()
        shipped_orders = (
            db.query(Order).filter(Order.status == ORDER_STATUS_SHIPPED).count()
        )
        delivered_orders = (
            db.query(Order).filter(Order.status == ORDER_STATUS_DELIVERED).count()
        )
        cancelled_orders = (
            db.query(Order).filter(Order.status == ORDER_STATUS_CANCELLED).count()
        )
        ...
        pending_inspections = (
            ...
            .filter(GeneralInspection.status == INSPECTION_STATUS_SCHEDULED)
            .count()
        )
        pending_agreements = (
            db.query(GeneralAgreement)
            .filter(GeneralAgreement.status == AGREEMENT_STATUS_PENDING_REVIEW)
            .count()
        )
        active_agreements = (
            db.query(GeneralAgreement)
            .filter(GeneralAgreement.status == AGREEMENT_STATUS_ACTIVE)
            .count()
        )

        # Real Estate stats
        total_internal_properties = (
            db.query(Property).filter(Property.title.ilike("[ACQUIRED]%")).count()
        )
        ...
        if range_start:
            revenue_rows = (
                ...
                .filter(
                    Payment.status == "completed", Payment.created_at >= range_start
                )
        ...
        recent_payments = [
            {
                ...
                "seller_name": (
                    p.seller.business_name if getattr(p, "seller", None) else None
                ),
...
async def get_admin_order_status_analytics(
    user=Depends(role_required(["admin"])), db: Session = Depends(get_db)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to fetch order status analytics"
        )
...
async def get_admin_user_details(
    ...
    try:
        target_user = (
            db.query(User).outerjoin(Profile).filter(User.id == user_id).first()
        )
        if not target_user:
        ...
        total_inspections = (
            db.query(GeneralInspection)
            .filter(GeneralInspection.user_id == user_id)
            .count()
        )
        total_agreements = (
            db.query(GeneralAgreement)
            .filter(GeneralAgreement.user_id == user_id)
            .count()
        )
        total_spent = (
            db.query(func.sum(Payment.amount))
...
async def get_admin_orders(
    ...
    try:
        ...
        # Get paginated results
        orders = (
            query.order_by(desc(Order.created_at), desc(Order.id))
            .offset(offset)
            .limit(limit)
            .all()
        )
...
async def get_admin_reviews(
    ...
    try:
        ...
        reviews = (
            query.order_by(desc(Review.created_at)).offset(offset).limit(limit).all()
        )
...
async def get_admin_order_details(
    ...
    try:
        ...
        order_payments = db.query(Payment).filter(Payment.order_id == order.id).all()
            ...
            "order_items": [
                {
                    ...
                    "product": {
                        ...
                        "images": [
                            {"id": str(img.id), "image_url": img.image_url}
...
async def mark_order_as_paid(
    ...
    try:
        ...
        return AdminResponse(
            success=True, message="Order marked as paid successfully", data=result
        )
    ...
    try:
        ...
        for order_id in order_ids:
            try:
                ...
                results.append(
                    {"order_id": str(order_id), "success": True, "data": result}
                )
                successful_count += 1
            except Exception as e:
                results.append(
                    {"order_id": str(order_id), "success": False, "error": str(e)}
                )
                failed_count += 1

        admin_logger.info(
            f"Bulk mark as paid: {successful_count} successful, {failed_count} failed by admin {user['id']}"
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to bulk mark orders as paid"
        )
...
async def update_product_status(
    ...
    try:
        admin_logger.info(
            f"Admin updating product {product_id} status to {request.action}"
        )
        ...
        admin_logger.info(
            f"Product {product_id} status updated to {product.status} by admin"
        )
...
async def update_user_action(
    ...
    try:
        ...
        if str(target_user.id) == user["id"]:
            raise HTTPException(
                status_code=400, detail="Cannot perform action on yourself"
            )
        ...
        if action.action == "lock_account":
            ...
            if action.lock_duration_hours:
                lock_until = datetime.utcnow() + timedelta(
                    hours=action.lock_duration_hours
                )
            else:
            ...
            message = (
                f"User account locked for {action.lock_duration_hours or 24} hours"
            )
        ...
        # Log the action
        admin_logger.info(
            f"Admin {user['id']} performed {action.action} on user {user_id}. Reason: {action.reason}"
        )
...
async def get_admin_inspections(
    ...
    try:
        ...
        if search:
            ...
            query = (
                query.join(GeneralInspection.user)
                .join(GeneralInspection.seller)
                .filter(
                    or_(
        ...
        total = query.count()
        inspections = (
            query.order_by(desc(GeneralInspection.created_at))
            .offset(offset)
            .limit(limit)
            .all()
        )
            ...
            data.append(
                {
                    ...
                    "seller_business_name": (
                        insp.seller.business_name if insp.seller else "N/A"
                    ),
                    "asset_title": (
                        insp.property.title
                        if insp.asset_type == "property" and insp.property
                        else (
                            f"{insp.car.brand} {insp.car.model}"
                            if insp.asset_type == "automotive" and insp.car
                            else "Asset"
                        )
                    ),
                }
...
async def get_admin_agreements(
    ...
    try:
        ...
        total = query.count()
        agreements = (
            query.order_by(desc(GeneralAgreement.created_at))
            .offset(offset)
            .limit(limit)
            .all()
        )
            ...
            data.append(
                {
                    ...
                    "total_paid": float(
                        (ag.total_price or 0) - (ag.remaining_balance or 0)
                    ),
                    ...
                    "seller_business_name": (
                        ag.seller.business_name if ag.seller else "N/A"
                    ),
                    "asset_title": (
                        ag.property.title
                        if ag.asset_type == "property" and ag.property
                        else (
                            f"{ag.car.brand} {ag.car.model}"
                            if ag.asset_type == "automotive" and ag.car
                            else "Asset"
                        )
                    ),
                }
...
async def get_admin_inspection(
    id: UUID, db: Session = Depends(get_db), user=Depends(role_required(["admin"]))
):
    try:
        inspection = (
            db.query(GeneralInspection).filter(GeneralInspection.id == id).first()
        )
        if not inspection:
        ...
        return {
            ...
            "data": {
                ...
                "agreed_price": (
                    float(inspection.agreed_price) if inspection.agreed_price else 0
                ),
                "status": inspection.status,
                "created_at": inspection.created_at.isoformat(),
                "user": {
                    "name": (
                        inspection.user.profile.name
                        if inspection.user.profile
                        else "N/A"
                    ),
                ...
                "seller": {
                    "business_name": (
                        inspection.seller.business_name if inspection.seller else "N/A"
                    )
                },
                "asset": {
                    "title": (
                        inspection.property.title
                        if inspection.asset_type == "property" and inspection.property
                        else (
                            inspection.car.brand + " " + inspection.car.model
                            if inspection.asset_type == "automotive" and inspection.car
                            else "Asset"
                        )
                    ),
                    "price": float(
                        ...
                        if inspection.asset_type == "property" and inspection.property
                            ...
                            inspection.car.price
                            if inspection.asset_type == "automotive" and inspection.car
                            else 0
                        )
                    ),
                    "image_url": (
                        inspection.property.images[0].image_url
                        if inspection.asset_type == "property"
                        and hasattr(inspection.property, "images")
                        and inspection.property.images
                        else (
                            inspection.car.images[0].image_url
                            if inspection.asset_type == "automotive"
                            and hasattr(inspection.car, "images")
                            and inspection.car.images
                            else None
                        )
                    ),
                },
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "total_paid": float(
                    (agreement.total_price or 0) - (agreement.remaining_balance or 0)
                ),
                ...
                "user": {
                    ...
                    "name": agreement.user.name or "N/A",
                },
                "seller": {
                    "business_name": (
                        agreement.seller.business_name if agreement.seller else "N/A"
                    )
                },
                "asset": {
                    "title": (
    ...
    try:
        ...
        if status_val:
            ...
            if agreement.asset_type == "property":
                ...
                prop = (
                    db.query(Property).filter(Property.id == agreement.asset_id).first()
                )
                if prop:
                    if status_val == "completed":
                        prop.status = (
                            "rented" if prop.listing_type == "rental" else "sold"
                        )
                ...
                if agreement.unit_id:
                    ...
                    unit = (
                        ...
                        .filter(PropertyUnit.id == agreement.unit_id)
                        .first()
                    )
                    if unit:
                        if status_val == "completed":
                            ...
                            unit.status = (
                                "rented"
                                if prop and prop.listing_type == "rental"
                                else "sold"
                            )
                        elif status_val == "active":
...
async def list_admin_properties(
    ...
    try:
        query = db.query(Property).options(
            joinedload(Property.seller), joinedload(Property.images)
        )
        ...
        if search:
            ...
            query = query.join(
                StoreProfile, Property.seller_id == StoreProfile.id
            ).filter(
                or_(
        ...
        offset = (page - 1) * limit
        paginated = (
            query.order_by(desc(Property.created_at)).offset(offset).limit(limit).all()
        )
...
async def list_admin_internal_inventory(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to fetch internal inventory"
        )
...
async def publish_acquired_property(
    ...
    try:
        ...
        if prop.status != "acquired":
            raise HTTPException(
                status_code=400,
                detail=f"Only properties in 'acquired' status can be published. Current status: {prop.status}",
            )
...
async def clear_system_cache(user=Depends(role_required(["admin"]))):
    ...
    try:
        ...
        return AdminResponse(
            success=True, message="System cache cleared successfully", data=None
        )
    except Exception as e:
...
async def get_system_logs(
    user=Depends(role_required(["admin"])), lines: int = Query(200, ge=10, le=1000)
):
    ...
    try:
        ...
        return AdminResponse(
            success=True, message="Logs retrieved successfully", data={"logs": logs}
        )
    except Exception as e:
...
async def preview_email_template(
    template_name: str, user=Depends(role_required(["admin"]))
):
    """
    Preview rendered email templates.
    Useful for testing and QA without sending actual emails.
    """
    ...
    mock_data = {
        ...
        "asset_title": "2023 Toyota Camry LE",
        "total": "₦4,500,000.00",
        "items_summary": "Toyota Camry LE x1",
        "reason": "Customer requested cancellation",
        "verification_code": "482910",
        "login_time": "14 Feb 2024, 09:30 AM",
        "ip_address": "192.168.1.1",
        "device": "macOS / Chrome",
        "dispute_title": "Item not as described",
        "resolution": "Refund issued",
        "tracking_note": "Your package is at the local sorting facility.",
        "subtotal": 4500000,
        "delivery_fee": 15000,
        "discount": 5000,
        "amount": 4510000,
        "remaining": "₦2,000,000.00",
        "next_due": "15 Mar 2024",
        "monthly": "₦200,000.00",
        "reference": "REF98765432",
    }

    try:
        if template_name == "verification":
            html, _ = email_service.render_verification_email(
                mock_data["user_name"], mock_data["verification_code"]
            )
        elif template_name == "order_confirmed":
            html, _ = email_service.render_order_confirmed_email(
                mock_data["user_name"],
                mock_data["order_id"],
                mock_data["items_summary"],
                mock_data["amount"],
                subtotal=mock_data["subtotal"],
                delivery_fee=mock_data["delivery_fee"],
                discount=mock_data["discount"],
            )
        elif template_name == "login":
            html, _ = email_service.render_login_email(
                mock_data["user_name"],
                mock_data["login_time"],
                mock_data["ip_address"],
                mock_data["device"],
            )
        elif template_name == "installment_reminder":
            html, _ = email_service.render_installment_reminder_email(
                mock_data["user_name"],
                mock_data["asset_title"],
                mock_data["monthly"],
                mock_data["next_due"],
                3,
                mock_data["remaining"],
            ...
            html, _ = email_service.render_notification_email(
                "system_announcement",
                "Test Generic Notification",
                "This is a generic notification fallback template.",
                mock_data["user_name"],
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Preview for template '{template_name}' not implemented.",
            )

        return HTMLResponse(content=html)
    except Exception as e:
        from core.logging_config import log_error, admin_logger

        log_error(admin_logger, f"Failed to render preview for {template_name}", e)
        raise HTTPException(
